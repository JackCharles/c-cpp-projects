...
set_up_gdt_descriptor:                      ;在GDT内安装一个新的描述符
         ...
         add ebx,[pgdt+2]                   ;下一个描述符的线性地址(偏移+基址)
...
;------------------------------------------------------------------------------- 
         ...
         ;任务控制块链
         tcb_chain        dd  0;每个任务有一个TCB，TCB用于记录任务更多的详细信息，
		                       ;与TSS不同TSS用于处理器访问，且只记录当前任务状态的必要信息，
							   ;而且是动态变化的，TCB是人为定义的，记录的信息方便我们自己使用
...
fill_descriptor_in_ldt:                     ;在LDT内安装一个新的描述符
         ...
         mov edi,[ebx+0x0c]                 ;获得LDT基地址(从对应TCB偏移0x0c处获取)
         
         xor ecx,ecx
         mov cx,[ebx+0x0a]                  ;获得LDT界限(LTD与TCB都是相对一个任务的)使用cx防止界限超过16位
         inc cx                             ;LDT的总字节数(LTD+1)，即新描述符偏移地址
         ...
         mov cx,ax
         shl cx,3                           ;左移3位，并且
         or cx,0000_0000_0000_0100B         ;使TI位=1，指向LDT，最后使RPL=00//RPL最终变到3
...
load_relocate_program:                      ;加载并重定位用户程序
         ...
         call sys_routine_seg_sel:allocate_memory
         mov [es:esi+0x06],ecx              ;登记程序加载基地址到TCB中(esi:TCB线性基址)
  ...
  .b1:
         ...
         mov ebx,0x000fffff                 ;以4k为单位，栈的上界
         sub ebx,ecx                        ;得到段界限
         mov eax,4096                        
         mul ecx                         
         ...
         add edi,0x28                       ;U-SALT在4GB段内的起始地址 
  .b2: 
         push ecx
         push edi
  ...
  .b3:
         ...
         mov ax,[esi+4]
         or ax,0000000000000011B            ;以用户程序自己的特权级使用调用门（表示该选择子由用户程序提供）
                                            ;故RPL=3 
  ...
  .b4:
         ...
         mov [es:esi+0x1a],ecx
         shr dword [es:esi+0x1a],12         ;登记0特权级堆栈尺寸到TCB (以4k为单位)shr 12=/4096
         call sys_routine_seg_sel:allocate_memory
         add eax,ecx                        ;堆栈必须使用高端地址为基地址
         mov [es:esi+0x1e],eax              ;登记0特权级堆栈基地址到TCB 
         mov ebx,0xffffe                    ;段长度（界限）(ffff ffff-1000)/fff
         mov ecx,0x00c09600                 ;4KB粒度，读写，特权级0
         call sys_routine_seg_sel:make_seg_descriptor
         mov ebx,esi                        ;TCB的基地址
         call fill_descriptor_in_ldt        ;加载到LTD中
         ...
         mov [es:esi+0x12],cx              
         dec word [es:esi+0x12]             ;登记TSS界限值到TCB -1
         call sys_routine_seg_sel:allocate_memory
         ...
         mov [es:ecx+102],dx                ;到TSS中 填的界限值表示不存在I/O许可位串
...
append_to_tcb_link:                         ;在TCB链上追加任务控制块
  ...
  .searc:
         mov edx,eax                        ;edx为TCB头
         mov eax,[es: edx+0x00]             ;如果是尾结点eax为0，否则eax为下一节点
         or eax,eax               
         jnz .searc
         
         mov [es: edx+0x00],ecx              ;添加到尾结点
         jmp .retpc
...
start:
  ...
  .b3:
         ...
         mov cx,1_11_0_1100_000_00000B      ;特权级3的调用门(3特权级以上程序的才
                                            ;允许访问该门)，0个参数(因为用寄存器
                                            ;传递参数，而没有用栈) 
         call sys_routine_seg_sel:make_gate_descriptor      ;构造门
         call sys_routine_seg_sel:set_up_gdt_descriptor     ;安装门（和安装GDT一样）
         mov [edi+260],cx                   ;将返回的门描述符选择子回填
		                                    ;公共例程段选择子被替换成门选择子,以便在后面重定位U-SALT时
											;回填调用门选择子，（用户只能通过门访问公共例程）
         ...
         ltr [ecx+0x18]                     ;加载任务状态段 ecx任务控制块地址
         lldt [ecx+0x10]                    ;加载LDT
