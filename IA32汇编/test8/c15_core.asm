...
terminate_current_task:                     ;终止当前任务
                                            ...
                                            ;一部分 
         pushfd                             ;EFLAGS寄存器压栈
         mov edx,[esp]                      ;从栈顶获得EFLAGS寄存器内容32位下独有做法
         add esp,4                          ;恢复堆栈指针

         mov eax,core_data_seg_sel
         mov ds,eax

         test dx,0100_0000_0000_0000B       ;测试NT位14位
         jnz .b1                            ;当前任务是嵌套的，到.b1执行iretd 14位不是0
         mov ebx,core_msg1                  ;当前任务不是嵌套的，直接切换到 
         call sys_routine_seg_sel:put_string
         jmp far [prgman_tss]               ;程序管理器任务，任务管理器TSS基址 
...
;------------------------------------------------------------------------------- 
         ...
         ;程序管理器的任务信息 
         prgman_tss       dd  0             ;程序管理器的TSS基地址；JMP返回时不会用它
...
start:
  ...
  .b3:
         ...
         ltr cx ;load TSS to task register                            
         ...
         ;ES指向0-4GB ecx为用户程序任务控制块基址，0x14偏向TSS选择子，32位偏移忽略掉
         call far [es:ecx+0x14]             ;执行任务切换。和上一章不同，任务切
                                            ;换时要恢复TSS内容，所以在创建任务
                                            ;时TSS要填写完整 
         ;////////////////////////////////////////////////////////////////////////////////                                
         ;重新加载并切换任务
         ;上一个用户程序并没有消失，因为没有回收内存		 
         mov ebx,prgman_msg2
         call sys_routine_seg_sel:put_string
         ;再次发起用户程序
         mov ecx,0x46
         ...
         jmp far [es:ecx+0x14]              ;执行任务切换；这次用的是jmp指令，不会发生嵌套
