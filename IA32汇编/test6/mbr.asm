  ...
  flush:                                  
   ...
   @2:
		 ...
		 ;加载完内核代码后就开始建立相应的描述符表
 setup:	                                    ;当前正在主引导程序代码段内，代码段不允许写入
         mov esi,[0x7c00+pgdt+0x02]         ;不可以在代码段内寻址pgdt，但可以
                                            ;通过4GB的段来访问
         ;建立公用例程段描述符
         mov eax,[edi+0x04]                 ;公用例程代码段起始汇编地址
         mov ebx,[edi+0x08]                 ;核心数据段汇编地址
         sub ebx,eax						;相减就是该段的长度
         dec ebx                            ;公用例程段界限，长度-1 
         add eax,edi                        ;公用例程段物理基地址,edi为内核代码物理基地址
         mov ecx,0x00409800                 ;字节粒度的代码段描述符
         call make_gdt_descriptor
         mov [esi+0x28],eax					;在上面五个描述符上继续添加
         mov [esi+0x2c],edx
         ...
         mov word [0x7c00+pgdt],63          ;描述符表的界限,总共8个描述符
                                        
         lgdt [0x7c00+pgdt]                 ;pgdt是相对于7c00的地址,绝对寻址是要加上7c00

         jmp far [edi+0x10]  				;跳转到内核开始处,edi内核基址
...
make_gdt_descriptor:                     ;构造描述符
                                         ...
                                         ;返回：EDX:EAX=完整的描述符
         mov edx,eax                     ;暂存eax
         shl eax,16                      ;高16位是基地址，低16位是段界限
         or ax,bx                        ;描述符低32位(EAX)构造完毕
      
         and edx,0xffff0000              ;清除基地址中无关的位，保留16到31位
         rol edx,8                       ;基地址分到两边23-16,31-24
         bswap edx                       ;装配基址的31~24和23~16  (80486+)交换
