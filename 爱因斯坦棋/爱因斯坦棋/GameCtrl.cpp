...
void GameCtrl::InitGameDate()
{
	...
	hCmpuTurn = LoadBitmap(m_hInstance, MAKEINTRESOURCE(IDB_BITMAP22));
	hYourTurn = LoadBitmap(m_hInstance, MAKEINTRESOURCE(IDB_BITMAP23));
}
...
void GameCtrl::ShowDice(HDC hMemDC1)
{
	...
	BitBlt(hMemDC1, 93, 44, 75, 75, hMemDC2, 0, 0, SRCCOPY);
	DeleteDC(hMemDC2);

}

void GameCtrl::ShowTurn(HDC hMemDC1)
{
	if (turn == 1)
	{
		HDC hMemDC2 = CreateCompatibleDC(hMemDC1);
		SelectObject(hMemDC2, hYourTurn);
		BitBlt(hMemDC1, 25, 220, 144, 74, hMemDC2, 0, 0, SRCCOPY);
		DeleteDC(hMemDC2);
	}
	else if (turn == 0)
	{
		HDC hMemDC2 = CreateCompatibleDC(hMemDC1);
		SelectObject(hMemDC2, hCmpuTurn);
		BitBlt(hMemDC1, 25, 220, 144, 74, hMemDC2, 0, 0, SRCCOPY);
		DeleteDC(hMemDC2);
	}
}
...
void GameCtrl::StartGame(int num)//num传入选定棋子的下标
{
	...
	{
		int rd = map[index_y + 1][index_x + 1];//右下方位置的棋子
		int down = map[index_y+1][index_x];//下方位置的棋子
		int right = map[index_y][index_x + 1];//右方位置的棋子
...
void GameCtrl::EatRightDown(int index_x, int index_y, int num, int rd)
{
	...
	map[index_y][index_x] = -1;//将移动的棋子从棋盘旧位置移开
	map[index_y + 1][index_x + 1] = Red[num].num + 6-1;//将移动的棋子放置在棋盘新位置
...
void GameCtrl::EatDown(int index_x, int index_y, int num, int down)
{
	...
	map[index_y][index_x] = -1;//将移动的棋子从棋盘旧位置移开
	map[index_y+1][index_x] = Red[num].num + 6-1;//将移动的棋子放置在棋盘新位置
	Red[num].y = (index_y + 1) * 100 + 5; //更新棋子的坐标
	nRedLeftCount--; //己方棋子数--
}
//吃掉右方己方棋子
void GameCtrl::EatRight(int index_x, int index_y, int num, int right)
{
	...
	map[index_y][index_x] = -1;//将移动的棋子从棋盘旧位置移开
	map[index_y][index_x + 1] = Red[num].num + 6-1;//将移动的棋子放置在棋盘新位置
